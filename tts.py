...
global convert_text_to_audio


class TextVoiceInput(BaseModel):
    text: str
    voice: Optional[str] = "zh-CN-YunyangNeural"
...
output_file = r"D:\ym_jobwoek_example\ER-NeRF\trial_vrh_torso\results\ngp_ep0023_ts_slices/output_audio.wav"
output_feature_file1 = "./output_features.npy"
...
parser.add_argument('--aud', type=str, default=output_feature_file1, help="audio source (empty will load the default, else should be a path to a npy file)")
...
@app.post("/convert/")
async def convert_text_to_audio(input_data: TextVoiceInput):
    ...
    try:
        ...
        convert_audio(input_file, output_file, target_sample_rate=16000)
        audio_ds(output_file, output_feature_file1)
        train_model(opt,model)
        return {"message": "Audio conversion successful!", "output_file": output_file, "output_feature_file": output_feature_file1}
...
def convert_audio(input_file, output_file, target_sample_rate=16000):
    ...
    aaaa=True
    """
    Convert audio file to the specified sample rate using ffmpeg.
    ...
    try:
        ...
        subprocess.run(["ffmpeg", "-i", input_file, "-ar", str(target_sample_rate), output_file], check=True)
...
def audio_ds(OUTPUT_FILE,output_feature_file):
    global output_feature_file1
    # 定义 DeepSpeech 模型文件路径
    deepspeech_model_file = 'deepspeech-0_1_0-b90017e8.pb'  # 或者你的 DeepSpeech 模型文件路径

    # 确保 DeepSpeech 模型文件存在
    if not os.path.exists(deepspeech_model_file):
        raise FileNotFoundError("DeepSpeech 模型文件不存在")
    
        # 调用提取特征的函数
    extract_features([OUTPUT_FILE], [output_feature_file], deepspeech_model_file)

    output_feature_file1 == output_feature_file
    # 输出提取的 DeepSpeech 特征文件路径
    print("DeepSpeech 特征已提取到:", output_feature_file)
